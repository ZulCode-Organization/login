...
export default function PerfilPage() {
  ...
  const conquistasMap = {
    "streak-7": { Icon: FlameIcon, label: "7 dias seguidos", color: "text-orange-400 bg-orange-400/10 border-orange-400/30" },
    "xp-100": { Icon: BoltIcon, label: "100 XP num dia", color: "text-yellow-400 bg-yellow-400/10 border-yellow-400/30" },
    "first-challenge": { Icon: TrophyIcon, label: "1º Desafio", color: "text-zul-blue bg-zul-blue/10 border-zul-blue/30" },
    "level-5": { Icon: StarIcon, label: "Nível 5", color: "text-gray-400 bg-gray-400/10 border-gray-400/20" },
    "challenges-10": { Icon: MonitorIcon, label: "10 Desafios", color: "text-gray-400 bg-gray-400/10 border-gray-400/20" },
    "advanced": { Icon: TrendingUpIcon, label: "Avançado", color: "text-gray-400 bg-gray-400/10 border-gray-400/20" },
  };

  const conquistas = user.conquistas.map((c: any) => ({
    ...conquistasMap[c.id as keyof typeof conquistasMap],
    desbloqueada: c.unlocked
  }));

  const statsList = [
    { label: "Dias seguidos", value: String(user.stats.streak), Icon: FlameIcon, color: "text-orange-400" },
    { label: "XP Total", value: user.xp.toLocaleString(), Icon: BoltIcon, color: "text-yellow-400" },
    { label: "Desafios", value: String(user.stats.challengesDone), Icon: TrophyIcon, color: "text-zul-blue" },
    { label: "Nível", value: String(user.level), Icon: StarIcon, color: "text-purple-400" },
  ];
        ...
        <main className="flex-1 pb-24 max-w-md mx-auto w-full px-4 pt-5">
           ...
           <div className="flex flex-col items-center gap-3 mb-6">
            <div className="relative">
              <div className="w-24 h-24 lg:w-28 lg:h-28 rounded-3xl bg-zul-blue shadow-neon-blue-lg flex items-center justify-center overflow-hidden">
                {user.avatar ? (
                  <img src={user.avatar} alt={user.name} className="w-full h-full object-cover" />
                ) : (
                  <span className="text-3xl lg:text-4xl font-black text-white tracking-tight">{user.initials}</span>
                )}
              </div>
          ...
          <Card className="p-4 mb-4">
            <div className="flex justify-between mb-2">
              <span className="text-xs text-gray-400 font-semibold">Progresso para Nível {user.level + 1}</span>
              <span className="text-xs font-bold text-zul-blue">{user.xp} / {user.nextLevelXp} XP</span>
            </div>
            <div className="w-full h-3 bg-zul-dark rounded-full overflow-hidden">
              <div className="h-full bg-zul-blue rounded-full" style={{ width: `${(user.xp / user.nextLevelXp) * 100}%` }} />
            </div>
          </Card>
          ...
          <Card className="p-4 mb-4">
            <div className="flex justify-between items-center mb-3">
              ...
              <span className="text-xs text-gray-500">{conquistas.filter(c => c.desbloqueada).length}/{conquistas.length}</span>
            </div>
      ...
      <div className="hidden lg:flex h-screen overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto px-8 py-8">
          <div className="max-w-2xl mx-auto">
             {/* Avatar Card e outros elementos seguem o mesmo padrão dinâmico */}
             <div className="flex flex-col items-center gap-3 mb-6">
                <div className="relative">
                <div className="w-24 h-24 lg:w-28 lg:h-28 rounded-3xl bg-zul-blue shadow-neon-blue-lg flex items-center justify-center overflow-hidden">
                    {user.avatar ? (
        ...
        <aside className="w-72 xl:w-80 flex flex-col flex-shrink-0 border-l border-zul-border overflow-y-auto px-4 py-6 gap-4">
          ...
          <div className="mt-auto">
            <div className="bg-zul-surface border border-zul-border rounded-2xl p-4 flex items-center gap-4">
              ...
              <div>
                <p className="text-lg font-black">{user.stats.streak} dias</p>
