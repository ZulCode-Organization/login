  ...
  return (
    <div className="min-h-screen bg-zul-dark flex flex-col items-center justify-center px-5 py-10">
      ...
      <div className="w-full max-w-sm bg-zul-surface border border-zul-border rounded-2xl p-6 shadow-2xl">
        <div className="flex bg-zul-dark rounded-xl p-1 mb-6">
          {(["login", "cadastro"] as const).map((t) => (
            <button key={t} onClick={() => setTab(t)}
              className={`flex-1 py-2.5 rounded-lg text-sm font-bold transition-all ${tab === t ? "bg-zul-blue text-white shadow-neon-blue" : "text-gray-500 hover:text-gray-300"}`}>
        ...
        <div className="flex flex-col gap-4">
          ...
          />
          {tab === "login" && (
            <button className="text-right text-xs text-zul-blue hover:underline">Esqueci minha senha</button>
          )}
          <Button variant="primary" size="lg" fullWidth onClick={handleSubmit} className="mt-2 uppercase tracking-wider">
            {tab === "login" ? "Entrar" : "Criar conta"}
          </Button>
        </div>

        <div className="flex items-center gap-3 my-5">
          <div className="flex-1 h-px bg-zul-border" />
          <span className="text-xs text-gray-600">ou continue com</span>
          <div className="flex-1 h-px bg-zul-border" />
        </div>

        <button className="w-full py-3 border border-zul-border rounded-xl flex items-center justify-center gap-3 text-sm font-semibold text-gray-300 hover:border-gray-600 hover:bg-zul-dark transition-all">
